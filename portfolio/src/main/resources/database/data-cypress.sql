...
DELETE FROM evidence_weblinks WHERE evidence_evidence_id IN (SELECT evidence_id FROM evidence WHERE project_id IN (SELECT project_id FROM project WHERE project_name LIKE '%Cypress%'));
